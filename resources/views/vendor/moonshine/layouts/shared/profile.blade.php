        ...
        <a href="{{ route('moonshine.custom_page', 'profile') }}" class="menu-profile-main">
            <div class="menu-profile-photo">
                <img class="h-full w-full object-cover"
                     src="{{ auth(config('moonshine.auth.guard'))->user()->avatar ? (Storage::disk('public')->url(auth(config('moonshine.auth.guard'))->user()->avatar)) : ("https://ui-avatars.com/api/?name=" . auth(config('moonshine.auth.guard'))->user()->name) }}"
